<x-app-layout>
    <x-admin.page>
        ...
        <x-admin.header title="Tambah Jenis Obat" subtitle="Buat kategori/jenis baru untuk obat.">
            <x-slot:actions>
        ...
        <x-admin.card class="p-5 sm:p-6">
            <form method="POST" action="{{ route('jenis-obat.store') }}" class="space-y-6">
                @csrf

                @include('jenis_obat._form')

                <div class="flex flex-col sm:flex-row gap-3 justify-end pt-2">
                    <x-admin.button variant="secondary" :href="route('jenis-obat.index')">
                        Batal
                    </x-admin.button>
